...
export const EcomProvider = ({ children }) => {
  ...
  const url = "https://hotel-api-1jnm.onrender.com";
  ...
  const signup = async (username, email, password) => {
    try {
      const response = await axios.post('https://hotel-api-1jnm.onrender.com/api/user/register', {
  ...
  const login = async (email, password) => {
    try {
      const response = await axios.post('https://hotel-api-1jnm.onrender.com/api/user/login', {
        email, password
      });
      const { user, token } = response.data;
